    ...
    AddToPlaylistDialogOnline(
        ...
        songs = remember { 
            songSelection.map { song ->
                ...
                val metadata = song.toMediaMetadata()
                com.cglabs.lifemusic.db.entities.Song(
                    song = com.cglabs.lifemusic.db.entities.SongEntity(
                        id = metadata.id,
                    ...
                    artists = metadata.artists.map { artist ->
                        com.cglabs.lifemusic.db.entities.ArtistEntity(
                            id = artist.id ?: "",
                            name = artist.name
                        )
                    },
                    album = metadata.album?.let { album ->
                        com.cglabs.lifemusic.db.entities.AlbumEntity(
                            id = album.id,
                            title = album.title,
                            thumbnailUrl = metadata.thumbnailUrl, 
                            songCount = 0,
                            ...
                            database.transaction {
                                songSelection.forEach { song ->
                                    ...
                                    if ((!allLiked && !metadata.liked) || allLiked) {
                                        ...
                                        val songEntity = com.cglabs.lifemusic.db.entities.SongEntity(
                                            id = metadata.id,
                                            title = metadata.title,
                                            duration = metadata.duration,
                                            thumbnailUrl = metadata.thumbnailUrl,
